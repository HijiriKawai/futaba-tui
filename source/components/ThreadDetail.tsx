...
export default function ThreadDetail({
	...
	function estimateResHeight(res: Res): number {
		...
		const imageCount =
			res.mediaUrls?.filter(url => /\.(jpe?g|png|gif|webp)$/i.test(url))
				.length || 0;
		const videoCount =
			res.mediaUrls?.filter(url => /\.(webm|mp4)$/i.test(url)).length || 0;
	...
	const resHeights = React.useMemo(
		() => filtered.map(estimateResHeight),
		[filtered],
	);

	// boxHeightに収まるだけレスを表示
	let total = 0;
	const visibleResponses: (Res | undefined)[] = [];
	...
	return (
		<Box flexDirection="column">
			...
			<Text color="yellow">
				全{responses.length}件中、{Math.min(scrollOffset + 1, responses.length)}
				〜{Math.min(scrollOffset + visibleResponses.length, responses.length)}
				件を表示中
				{hideDeletedRes &&
					responses.length - filtered.length > 0 &&
			...
			{urlSelectMode && urlSelectMode.urls.length > 1 && (
				...
				>
					<Text color="yellow">
						URLを選択してください（1〜{urlSelectMode.urls.length}
						、q/escでキャンセル）
					</Text>
