...
export const defaultConfig = {
	...
	helpText: {
		...
		threadList:
			'スレッド一覧（{up}{down}{left}{right}:移動 {enter}:詳細 {back}:板選択 {reload}:リロード {openImage}:画像 {sortPrev}{sortNext}:ソート {history}:履歴 {quit}:終了）',
		threadDetail:
			'レス一覧（{up}{down}:移動 {back}:戻る {reload}:リロード {quoteModal}:引用 {openImage}:画像 {openLink}:リンク {toggleDeleted}:削除非表示 {history}:履歴 {quit}:終了）',
		historyList:
			'履歴一覧（{up}{down}:移動 {enter}:開く {back}:戻る {quit}:終了 {clearHistory}:クリア {toggleHistoryAll}:全表示）',
		urlSelectModal:
			'URLを選択してください（数字:選択 {urlSelectCancel}/{urlSelectCancelEsc}:キャンセル）',
		quoteModal:
